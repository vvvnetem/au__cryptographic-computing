...
public class ElGamal {
    ...
    public Ciphertext encrypt(BigInteger message, PublicKey publicKey) {
        /*
        When you encrypt using ElGamal you need to encode the message
        m < p into an element of the group generated by g.
        The best method was given in the cryptography notes: If you want to encrypt m, check if (m+1)^q =
        1 mod p. If yes, encrypt M = m + 1. If not, encrypt M = −(m + 1). When you decrypt, if M ≤ q
        output m = M − 1m otherwise output m = −M − 1.
         */
        ...
        if (s.equals(BigInteger.ONE)) {
            //M = m + 1 mod p
            message = message.add(BigInteger.ONE).mod(p);
        } else {
            //M = −(m + 1) mod p
            message = message.add(BigInteger.ONE).negate().mod(p);
        }
    ...
    public BigInteger decrypt(Ciphertext ciphertext, SecretKey secretKey) {
        ...
        // M = c2 * (c1^-x) mod p
        BigInteger M = ciphertext.d.multiply(cxInv).mod(p);
