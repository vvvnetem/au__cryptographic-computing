 ...
 *
 * Notations are choosed accordingly to previous boolean circuits:
...
public class Alice {
    ...
    private ElGamalOT.ReceiverState[] otStates = new ElGamalOT.ReceiverState[3];
     ...
     */
    public ElGamalOT.ReceiverState prepareOTReceiver(ElGamalOT ot, int i, java.math.BigInteger C) {
        ElGamalOT.ReceiverState st = ot.receiverGenerateBetas(inputBits[i], C);
        otStates[i] = st;
        return st;
    }

    /** After Bob responds with SenderOutput, recover chosen label */
    public byte[] recoverFromOT(ElGamalOT ot, int i, ElGamalOT.SenderOutput so) throws Exception {
        ElGamalOT.ReceiverState st = otStates[i];
