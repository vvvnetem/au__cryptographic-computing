...
while at most $q_{H_2}, q_{H_3}, q_{H_4}$ queries for the hash functions $H_2, H_3, H_4$ respectively. Thus the BDH algorithm (denoted as $\mathbb{B}$) for $\mathcal{G}$ with running time at most $t(k)$ can be  written as follows \cite{BF01} in \textit{Equation \ref{eq:hashcca}}: 
