...
\subsubsection{Choosing Type-3 elliptic curves}
...
the problem into the $\mathbb{F}_{p^k}$ ﬁeld is useless for an attacker.
...
To mitigate such attacks, we note that execution a more  costly key derivation function  (e.g., \textit{PBKDF2}, \textit{scrypt}, or \textit{Argon2})  on the password, then hashing the output of that function to the target \textit{BLS12-381} elliptic curve in accordance with \textit{Section 5.3.1.} of \cite{ecc.hashing} can be used as a further extension of our solution. \cite{ecc.hashing}
